"""Generate ISSUES.md — known formula-level problems in the Zimbabwe workbook.
...
Inputs: temp/Zimbabwe BOOST.xlsx, defined_names.csv, formula_map.csv, hardcoded_columns.csv, hardcoded_cells.csv
Output: Zimbabwe/_analysis/ISSUES.md
"""
...
# Measure-agnostic normalizer: collapse approved/executed (and the _r revenue
# counterparts) to a common token so an Approved-sheet SUMIFS over `approved`
# compares equal to the same Executed-sheet SUMIFS over `executed`.
_MEASURE_RE = re.compile(r"\b(approved_orig|approved|executed_r|executed)\b")
...
def main():
    ...
    # ---------- write ISSUES.md ----------
    L: list[str] = []
    L.append("# Zimbabwe BOOST — Known Formula Issues\n")
    ...
    if col_overrides:
        L.append(f"**{len(col_overrides)} columns** in Approved/Executed contain hand-entered values mixed with SUMIFS formulas. The pipeline will emit the SUMIFS result; cells where Excel has a literal will appear as discrepancies.\n")
        L.append("| Sheet | Column | #formula | #literal | Sample literals |")
        L.append("|---|---|---:|---:|---|")
        for r in col_overrides[:60]:
            samp = (r["literal_samples"] or "")[:120]
            label = r["header"] or r["col_letter"]
            L.append(f"| {r['sheet']} | {label} | {r['n_formula']} | {r['n_literal_numeric']} | {samp} |")
        if len(col_overrides) > 60:
            L.append(f"\n_…and {len(col_overrides)-60} more. Full list: `hardcoded_columns.csv`._")
        L.append(f"\n**Cell-level detail:** `hardcoded_cells.csv` ({len(hardcoded_cells)} cells).")
    else:
        L.append("_None._")
    ...
    L.append("Columns in Approved/Executed where the formula shape is not uniform across rows. The pipeline applies one rule per column; rows using a different shape need expert classification.\n")
    if inconsistent_cols:
        L.append("| Sheet | Column | #cells | #distinct shapes | Top shape % |")
        L.append("|---|---|---:|---:|---:|")
        for r in inconsistent_cols[:40]:
            label = col_header.get((r["sheet"], r["col_idx"]), r["col_letter"])
            L.append(f"| {r['sheet']} | {label} | {r['n_cells']} | {r['n_shapes']} | {r['top_shape_pct']} |")
        if len(inconsistent_cols) > 40:
            L.append(f"\n_…and {len(inconsistent_cols)-40} more._")
    else:
        L.append("_None — every column uses a single formula shape._")
    ...
    L.append(f"- _Ignored: {measure_only_diffs} cells where the only difference was `approved` vs `executed` named-range — that's expected._")
    if shape_diffs:
        L.append("\nFirst 20 shape differences:\n")
        L.append("| Row | Column | Approved | Executed |")
        L.append("|---:|---|---|---|")
        for (r, c), af, ef in shape_diffs[:20]:
            label = col_header.get(("Approved", c)) or col_header.get(("Executed", c)) or get_column_letter(c)
            L.append(f"| {r} | {label} | `{af[:80]}` | `{ef[:80]}` |")
