...
with open(
    f"{RAW_INPUT_DIR}/{COUNTRY}/2023/labels_en_v01_overall.json", "r"
) as json_file:
...
def boost_bronze():
    return (
        ...
        .load(f"{COUNTRY_MICRODATA_DIR}/Data_Expenditures.csv").withColumn(
            "id",concat(lit("alb_2_"), monotonically_increasing_id()))
    )
...
def boost_silver():
    silver_df = (
        (dlt.read(f"alb_2023_onward_boost_bronze"))
        ...
        .withColumn(
            "admin3",
            when((col("admin1") == "2") & (col("admin3") == 0), 100).otherwise(
                col("admin3")
            )
            # econ tagging
        )
        ...
        .withColumn(
            ...
            when(
                col("econ5").isNotNull(),
                substring(col("econ5").cast("string"), 1, 1).cast("int"),
        ...
        .withColumn(
            ...
            when(
                col("econ5").isNotNull(),
                substring(col("econ5").cast("string"), 1, 2).cast("int"),
        ...
        .withColumn(
            ...
            when(
                col("econ5").isNotNull(),
                substring(col("econ5").cast("string"), 1, 4).cast("int"),
            ),
        )
        ...
        .withColumn(
            ...
            when(
                col("executed").isNull(),
                substring(col("econ3").cast("string"), 1, 1).cast("int"),
            ).otherwise(col("econ1")),
        )
        .withColumn(
            "econ2",
            when(
                col("executed").isNull(),
                substring(col("econ3").cast("string"), 1, 2).cast("int"),
            ).otherwise(col("econ2"))
            # functional tagging
        )
        ...
        .withColumn(
            ...
            col("func3").cast(
                "double"
            ),  # substituting with values from func3_n for those where the code is alphanumeric
        )
        .withColumn(
            "func3_n",
            when(
                (col("year") == 2023) & col("func3").isNull() & (col("program1") != ""),
        ...
        .withColumn(
            "func3_n",
            when(
                col("func3_n").isNotNull(), concat(col("func3_n"), lit("0"))
            ).otherwise(col("func3_n")),
        )
        .withColumn(
            "func3_n",
            when(
                col("func3_n").isin(["0140", "0430", "0660", "0910"]),
        ...
        .withColumn(
            "exp_type",
            when(col("econ3").isin([600, 601]), 1).otherwise(col("exp_type")),
        )
        .withColumn(
            "exp_type",
            when(
                col("econ2").isin([65, 66])
                | col("econ3").isin([602, 603, 604, 605, 606]),
                2,
        ...
        .withColumn(
            "exp_type",
            when((col("econ2") == 67) | (col("econ3") == 609), 4).otherwise(
                col("exp_type")
            )
            # project source
        )
        ...
        .withColumn(
            "project_s",
            when(
                col("project_s").isNull() & (col("project_source") != "."), 1
        ...
        .withColumn(
            ...
            when(
                ...
                | col("econ5").isin(
                    [6040001, 6040002, 6040006, 6040007, 6040010, 6041100]
                ),
                1,
            )
    ...
    for column_name, mapping in labels.items():
        if column_name in silver_df.columns:
            silver_df = silver_df.withColumn(
                column_name, replacement_udf(column_name)(col(column_name))
            )

    silver_df = (
        silver_df.filter(col("transfer") == "Excluding transfers")
        .withColumn("is_foreign", col("fin_source").startswith("2"))
        .withColumn(
            "admin0",
            when(
                col("admin2").startswith("00") | col("admin2").startswith("999"),
                "Central",
            ).otherwise("Regional"),
        )
        .withColumn(
            "admin1_tmp",
            when(col("counties") == "Central", "Central Scope").otherwise(
                col("counties")
            ),
        )
        .withColumn(
            "admin2_tmp",
            when(col("counties") == "Central", col("admin3")).otherwise(
                col("counties")
            ),
        )
        ...
        .withColumn(
            ...
            # public safety
            .when(
                col("func2").substr(1, 3).isin(["031", "034", "035"]), "public safety"
            )
            # spending in energy
            .when(col("func2").startswith("043"), "Energy")
            # primary and secondary health
            .when(
                col("func2").startswith("072") | col("func2").startswith("074"),
                "primary and secondary health",
            )
            ...
            # primary education
            .when(
                col("func1").startswith("09") & col("func2").startswith("091"),
                "primary education",
            )
            # secondary education
            .when(
                col("func1").startswith("09") & col("func2").startswith("092"),
                "secondary education",
            )
            # tertiary education
            .when(
                col("func1").startswith("09") & col("func2").startswith("094"),
                "tertiary education",
            ),
        )
        .withColumn(
        ...
        .withColumn(
            ...
            # basic wages
            .when(
                col("econ3").startswith("600") | col("econ3").startswith("601"),
                "basic wages",
            )
            # pension contributions
            .when(
                col("econ3").startswith("601"),
                "social benefits (pension contributions)",
            )  # note this will be zero since it is subsumed into above category
            # capital expenditures (foreign funded)
            .when(
                col("is_foreign") & col("exp_type").startswith("3"),
                "capital expenditure (foreign funded)",
            )
            # no entry for capital maintenance
            # goods and services (basic services)
            .when(
                col("econ4").startswith("6022") | col("econ4").startswith("6026"),
                "basic services",
            )
            ...
            # social assistance
            .when(
                col("econ3").startswith("606") & col("func2").startswith("104"),
                "social assistance",
            )
            # pensions
            .when(
                col("econ3").startswith("606") & col("func2").startswith("102"),
                "pensions",
            )
            # other social benefits
            .when(
                col("econ3").startswith("606") & col("func2").startswith("10"),
                "other social benefits",
            ),  # should come after social assistance and pensions
        )
        .withColumn(
            "econ",
            # wage bill
            when(
                col("econ3").startswith("601") | col("econ3").startswith("600"),
                "Wage bill",
            )
            # capital expenditure
            ...
            .when(
                col("econ3").startswith("604") | col("econ3").startswith("605"),
                "Other grants and transfers",
            )
            # interest on debt
            .when(
                col("econ2").startswith("65") | col("econ2").startswith("66"),
                "Interest on debt",
            )
            # other expenses
...
def boost_silver():
    return (
        dlt.read(f"alb_2022_and_before_boost_bronze")
...
def alb_2022_and_before_boost_gold():
    return (
        dlt.read(f"alb_2022_and_before_boost_silver")
...
def alb_2023_onward_boost_gold():
    return (
        dlt.read(f"alb_2023_onward_boost_silver")
        .withColumn("country_name", lit(COUNTRY))
        .select(
            "id",
            "country_name",
