...
# Column layout of Approved/Executed: col A = tag code, col B = human category.
CODE_COL = 1
CATEGORY_COL = 2
...
def main():
    ...
    for sheet in TARGET_SHEETS:
        ...
        # Per-row code and category lookups (column A / column B cached values).
        row_code: dict[int, str] = {}
        row_category: dict[int, str] = {}
        for idx, row_v in enumerate(v_grid, start=1):
            if idx <= HEADER_ROWS:
                continue
            if CODE_COL - 1 < len(row_v):
                v = row_v[CODE_COL - 1]
                if v is not None:
                    row_code[idx] = str(v)
            if CATEGORY_COL - 1 < len(row_v):
                v = row_v[CATEGORY_COL - 1]
                if v is not None:
                    row_category[idx] = str(v)
                ...
                if is_formula:
                    ...
                    if isinstance(v, (int, float)) and not isinstance(v, bool):
                        ...
                        cell_rows.append({
                            ...
                            "code": row_code.get(r_idx, ""),
                            "category": row_category.get(r_idx, ""),
                            "value": v,
    ...
    with (OUT / "hardcoded_cells.csv").open("w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            "sheet","cell","row","col_letter","col_idx","header",
            "code","category","value","type",
        ])
        w.writeheader()
