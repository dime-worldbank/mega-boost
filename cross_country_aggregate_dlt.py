...
schema = StructType([
    StructField("index", IntegerType(), True, {'comment': 'id to each budget item'}),
...
def boost_gold_test():
    ...
    for code in country_codes:
        ...
        for col_name in ["func", "func_sub", "econ", "econ_sub", "admin0", "admin1", "admin2", "geo1", "revised", "is_foreign", "index"]:
...
def cpi_factor():
    earliest_years = (dlt.read('boost_gold_test')
...
@dlt.table(name=f'country_expenditure_enriched')
def country_expenditure_enriched():
    boost_gold = dlt.read('boost_gold_test')
    ...
    return (boost_gold
        .withColumnRenamed("executed", "expenditure")
        .withColumnRenamed("approved", "budget")
...
def expenditure_by_country_year():
    boost_gold = dlt.read('country_expenditure_enriched').dropDuplicates(subset=["country_name", "index"])
    
    return (boost_gold
        .groupBy("country_name", "year").agg(
            F.sum("expenditure").alias("expenditure"),
            F.sum(
                F.when(boost_gold["admin0"] == "Regional", boost_gold["expenditure"])
            ).alias("decentralized_expenditure"),
            F.sum(
                F.when(boost_gold["is_foreign"], boost_gold["expenditure"])
            ).alias("foreign_funded_expenditure"),
            F.sum("budget").alias("budget"),
            F.sum(
                F.when(boost_gold["admin0"] == "Regional", boost_gold["budget"])
            ).alias("decentralized_budget"),
            F.sum(
                F.when(boost_gold["is_foreign"], boost_gold["budget"])
...
#     boost_gold = dlt.read('boost_gold_test').dropDuplicates(subset=["country_name", "index", "func"])
#     window_spec = Window.partitionBy("country_name")
...
#     with_decentralized = (dlt.read('boost_gold_test').dropDuplicates(subset=["country_name", "index", "func", "func_sub", "econ", "econ_sub"])
#         .withColumn("is_foreign", F.when(F.col("is_foreign").isNull(), False).otherwise(F.col("is_foreign")))
#         .groupBy("index", "country_name", "year", "admin0", "admin1", "admin2", "func", "func_sub", "econ", "econ_sub", "is_foreign").agg(
#             F.sum("executed").alias("expenditure"),
#             F.sum("approved").alias("budget")
#         )
...
#     with_decentralized = (dlt.read('boost_gold_test').dropDuplicates(subset=["country_name", "index", "func", "func_sub"])
#         .groupBy("index","country_name", "year", "geo0", "func", "func_sub").agg(
#             F.sum("executed").alias("expenditure"),
#             F.sum("approved").alias("budget")
#         )
...
#     return (dlt.read('boost_gold_test').dropDuplicates(subset=["country_name","index", "func", "econ"])
#         .groupBy("index", "country_name", "year", "func", "econ")
#         .agg(
#             F.sum("executed").alias("expenditure"),
#             F.sum("real_expenditure").alias("real_expenditure"),
#             F.sum(
...
#     return (dlt.read(f'boost_gold_test').dropDuplicates(subset=["country_name", "index", "func"])
#         .groupBy("index","country_name", "year", "func").agg(
#             F.sum("executed").alias("expenditure"),
#             F.sum("real_expenditure").alias("real_expenditure"),
...
#     return (dlt.read('boost_gold_test').dropDuplicates(subset=["country_name", "index", "econ"])
#         .groupBy("country_name", "index", "year", "econ").agg(
#             F.sum("executed").alias("expenditure"),
#             F.sum("real_expenditure").alias("real_expenditure"),
#             F.sum("decentralized_expenditure").alias("decentralized_expenditure"),
#             F.sum("central_expenditure").alias("central_expenditure"),
...
#     quality_cci_total = (spark.table(f'{catalog}.{quality_source_schema}.quality_total_gold')
...
def quality_boost_func_sub_exact():
    ...
    # without the CCI excel being updated.
    boost_countries = dlt.read('quality_boost_country').select('country_name').distinct()
    quality_cci_func = (spark.table(f'{catalog}.{quality_source_schema}.quality_functional_sub_gold')
    ...
    return (
        ...
        .groupBy('country_name', 'func_sub')
        .agg(F.count('*').alias('row_count'))
        .join(quality_cci_func, on=['country_name', 'func_sub'], how="left")
    )
...
#         .groupBy('country_name', 'econ_sub')
#         .agg(F.count('*').alias('row_count'))
#         .join(quality_cci_econ, on=['country_name', 'econ_sub'], how="left")
#     )
...
#     boost_countries = dlt.read('quality_boost_country').select('country_name').distinct()
#     quality_cci_foreign = (spark.table(f'{catalog}.{quality_source_schema}.quality_total_foreign_gold')
#         .groupBy('country_name')
#         .agg(F.count('*').alias('cci_row_count'))
