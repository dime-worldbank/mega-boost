---
name: onboard-country
description: >-
  Onboard a new country into the mega-boost BOOST pipeline. Use when the user
  wants to add/onboard/ingest a country's BOOST Excel workbook — scaffolding the
  country folder, extracting tag rules and the code dictionary, running the
  overlap/discrepancy audits, building the DLT notebooks, and registering it in
  the cross-country aggregate. Zimbabwe is the reference implementation; Moldova
  covers the harder edge cases (multi-sheet raw data, bilingual filters,
  meta-rollups).
---

# Onboarding a new country — AI workflow guide

This skill tells an AI agent (Claude Code, or any coding assistant) how to
